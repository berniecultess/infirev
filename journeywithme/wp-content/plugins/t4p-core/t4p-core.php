<?php
/*
Plugin Name: Theme4Press Core
Plugin URI: http://www.theme4press.com
Description: Theme4Press Core Plugin for Theme4Press Themes
Version: 1.3.3
Author: Theme4Press
...
if( ! class_exists( 'T4PCore_Plugin' ) ) {
	class T4PCore_Plugin {
		 ...
		 * @var      string
		 */
		protected $plugin_slug = 't4p-core';
		...
		private function __construct() {
    	  define('T4P_TINYMCE_URI', plugin_dir_url( __FILE__ ) . 'tinymce');
		    define('T4P_TINYMCE_DIR', plugin_dir_path( __FILE__ ) .'tinymce'); 
        add_action('init', array(&$this, 'init'));
        add_action('admin_init', array(&$this, 'admin_init'));
		add_action('after_setup_theme', array(&$this, 'load_t4p_core_text_domain'));		
        add_action('wp_ajax_t4p_shortcodes_popup', array(&$this, 'popup'));
	}
	 ...
	 */
		...
		/*if( ! is_admin() )
		{
			wp_enqueue_style( 't4p-shortcodes', plugin_dir_url( __FILE__ ) . 'shortcodes.css' );
			wp_enqueue_script( 'jquery-ui-accordion' );
			wp_enqueue_script( 'jquery-ui-tabs' );
			wp_enqueue_script( 't4p-shortcodes-lib', plugin_dir_url( __FILE__ ) . 'js/t4p-shortcodes-lib.js', array('jquery', 'jquery-ui-accordion', 'jquery-ui-tabs') );
		}*/
		 ...
		 * @return void
		 */		
		function load_t4p_core_text_domain() {
			load_plugin_textdomain( 't4p-core', false, dirname( plugin_basename(__FILE__) ) . '/languages' );
		}		
		...
		public static function attributes( $slug, $attributes = array() ) {
			...
			$out = '';
			$attr = apply_filters( "t4p_attr_{$slug}", $attributes );
		...
		public static function animations( $args = array() ) {
			...
			if( $args['type'] ) {
				...
				$animation_attribues['animation_class'] = 't4p-animated';
	...
	{
		if( is_admin() ) {
			$plugin_array['t4p_button'] = T4P_TINYMCE_URI . '/plugin.js';
		}
	...
	{
		// css
		wp_enqueue_style( 't4p-popup', T4P_TINYMCE_URI . '/css/popup.css', false, '1.0', 'all' );
		wp_enqueue_style( 'jquery.chosen', T4P_TINYMCE_URI . '/css/chosen.css', false, '1.0', 'all' );
		wp_enqueue_style( 'font-awesome', T4P_TINYMCE_URI . '/css/font-awesome.css', false, '4.0.3', 'all' );
		wp_enqueue_style( 'wp-color-picker' );

		// js
		wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_script( 'jquery-livequery', T4P_TINYMCE_URI . '/js/jquery.livequery.js', false, '1.1.1', false );
		wp_enqueue_script( 'jquery-appendo', T4P_TINYMCE_URI . '/js/jquery.appendo.js', false, '1.0', false );
		wp_enqueue_script( 'base64', T4P_TINYMCE_URI . '/js/base64.js', false, '1.0', false );
		wp_enqueue_script( 'jquery.chosen', T4P_TINYMCE_URI . '/js/chosen.jquery.min.js', false, '1.0', false );
    	wp_enqueue_script( 'wp-color-picker' );

		wp_enqueue_script( 't4p-popup', T4P_TINYMCE_URI . '/js/popup.js', false, '1.0', false );

		// Developer mode
		$dev_mode = current_theme_supports( 't4p_shortcodes_embed' );
		if( $dev_mode ) {
			$dev_mode = 'true';
		} else {
			$dev_mode = 'false';
    ...
    wp_localize_script( 't4p-popup', 't4pShortcodes', array('plugin_folder' => plugins_url( '', __FILE__ ), 'dev' => $dev_mode) );
	}
...
/**
	...
	function popup() {
		...
		require_once( T4P_TINYMCE_DIR . '/t4p-sc.php' );
		...
		/*function updater() {
			...
			$theme_update = new T4PCoreUpdater( 'http://updates.theme-t4p.com/t4p-core.php', $plugin_id, $plugin_slug );
		}*/
	}
}
// Load the instance of the plugin
add_action( 'plugins_loaded', array( 'T4PCore_Plugin', 'get_instance' ) );

/**
 * T4P Slider
 */
include_once 't4p-slider.php';
...
if ('evolve Plus' == $theme->name || 'evolve Plus' == $theme->parent_theme) {
     add_action( 'init', 't4p_register_post_types' );
function t4p_register_post_types() {
	global $smof_data;
	global $evl_options;
	...
	register_post_type(
		...
		array(
			...
			'has_archive' => false,
			'rewrite' => array(
				'slug' => $smof_data['portfolio_slug'].$evl_options['evl_portfolio_slug']
			),
	...
	register_taxonomy('portfolio_category', 'evolve_portfolio', array('hierarchical' => true, 'label' => 'Categories', 'query_var' => true, 'rewrite' => true));
	register_taxonomy('portfolio_skills', 'evolve_portfolio', array('hierarchical' => true, 'label' => 'Skills', 'query_var' => true, 'rewrite' => true));
	register_taxonomy('portfolio_tags', 'evolve_portfolio', array('hierarchical' => false, 'label' => 'Tags', 'query_var' => true, 'rewrite' => true));
	$theme = wp_get_theme(); // gets the current theme
        if ('evolve Plus' == $theme->name || 'evolve Plus' == $theme->parent_theme) {
		} else {
	register_post_type(
	...
	register_taxonomy('faq_category', 'alora_faq', array('hierarchical' => true, 'label' => 'Categories', 'query_var' => true, 'rewrite' => true));
	...
	if( ! $smof_data['status_eslider'] ) {
		...
		register_post_type(
			...
			array(
				...
				'menu_position' => 100,
				'labels' => array(
					'name'                => _x( 'Elastic Sliders', 'Post Type General Name', 't4p-core' ),
					'singular_name'       => _x( 'Elastic Slider', 'Post Type Singular Name', 't4p-core' ),
					'menu_name'           => __( 'Elastic Slider', 't4p-core' ),
		...
		register_taxonomy(
			...
			array(
				...
				'rewrite' => true,
				'labels' => array(
					'name'                       => _x( 'Groups', 'Taxonomy General Name', 't4p-core' ),
					'singular_name'              => _x( 'Group', 'Taxonomy Singular Name', 't4p-core' ),
					'menu_name'                  => __( 'Add or Edit Groups', 't4p-core' ),
					'all_items'                  => __( 'All Groups', 't4p-core' ),
					'parent_item'                => __( 'Parent Group', 't4p-core' ),
					'parent_item_colon'          => __( 'Parent Group:', 't4p-core' ),
					'new_item_name'              => __( 'New Group Name', 't4p-core' ),
					'add_new_item'               => __( 'Add Groups', 't4p-core' ),
					'edit_item'                  => __( 'Edit Group', 't4p-core' ),
					'update_item'                => __( 'Update Group', 't4p-core' ),
					'separate_items_with_commas' => __( 'Separate groups with commas', 't4p-core' ),
					'search_items'               => __( 'Search Groups', 't4p-core' ),
					'add_or_remove_items'        => __( 'Add or remove groups', 't4p-core' ),
					'choose_from_most_used'      => __( 'Choose from the most used groups', 't4p-core' ),
					'not_found'                  => __( 'Not Found', 't4p-core' ),
				),
			)
		);
...
function t4p_register_post_types() {
	...
	register_taxonomy('portfolio_category', 'alora_portfolio', array('hierarchical' => true, 'label' => 'Categories', 'query_var' => true, 'rewrite' => true));
	register_taxonomy('portfolio_skills', 'alora_portfolio', array('hierarchical' => true, 'label' => 'Skills', 'query_var' => true, 'rewrite' => true));
	register_taxonomy('portfolio_tags', 'alora_portfolio', array('hierarchical' => false, 'label' => 'Tags', 'query_var' => true, 'rewrite' => true));
...
add_action( 'admin_menu', 't4p_admin_menu' );
function t4p_admin_menu() {
	global $submenu;

	unset( $submenu['edit.php?post_type=theme4press_elastic'][10] );
}

/*----------------------------------------------------------------------------*
* Remove extra P tags
*----------------------------------------------------------------------------*/
function t4p_shortcodes_formatter($content) {
...
add_filter('the_content', 't4p_shortcodes_formatter');
add_filter('widget_text', 't4p_shortcodes_formatter');
