<div class="wrap">
	...
	<div id="poststuff">
		...
		<div id="post-body" class="metabox-holder columns-2">
            ...
            <input name="ml2_remove" value="1" type="hidden" />
            			<?php 
						if ($_SERVER['REQUEST_METHOD']=="POST" and $_POST['ml2_remove']) {
							if ($_GET['rem']) $_POST['rem'][] = $_GET['rem'];
							$count = 0;
							if (is_array($_POST['rem'])) {
								foreach ($_POST['rem'] as $id) { 
									$wpdb->query("delete from ".$wpdb->prefix."ml2 where id = '".$wpdb->escape($id)."' limit 1"); 
									$count++; 
								}
						...
						if ($_SERVER['REQUEST_METHOD']=="POST" and $_POST['ml2_import']) {
							$correct = 0;
									...
									if (count($lines)) {
										...
										foreach ($lines as $data) {
											...
											if (is_email(trim($data[0]))) {
												$c = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."ml2 where ml2_email LIKE '".$wpdb->escape(trim($data[0]))."' limit 1", ARRAY_A);
												if (!is_array($c)) {											
													$wpdb->query("INSERT INTO ".$wpdb->prefix."ml2 (ml2_email, ml2_name) VALUES ('".$wpdb->escape(trim($data[0]))."', '".$wpdb->escape(trim($data[1]))."')");
													$correct++;
												} else { $exists++; }
											} else { $invalid++; }
										}
						...
						<table cellspacing="0" class="wp-list-table widefat fixed subscribers">
                          <thead>
                            <tr>
                                ...
                                <th style="" class="manage-column column-phone" id="phone" scope="col"><span>Phone</span><span class="sorting-indicator"></span></th>
                            </thead>
                            ...
                            <tr>
                                ...
                                <th style="" class="manage-column column-phone" scope="col"><span>Phone</span><span class="sorting-indicator"></span></th>
                            </tfoot>
                            ...
                            <?php 
								...
								$results = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix."ml2");
								if (count($results)<1) echo '<tr class="no-items"><td colspan="3" class="colspanchange">No mailing list subscribers have been added.</td></tr>';
									...
									foreach($results as $row) {
										...
										echo '<tr>
													...
													<td>'.esc_js(esc_html($row->ml2_name)).'</td>
  													<td>'.esc_js(esc_html($row->ml2_email)).'</td>
  													<td>'.esc_js(esc_html($row->ml2_phone)).'</td>
											  </tr>';
                ...
                <input name="ml2_import" value="1" type="hidden" />
					...
					<div class="meta-box-sortables">
			               <div class="postbox">
			                        ...
			                        <div class="inside">
										...
										<p>2- Add [ml2subform] on any page / post you'd like</p>
										<p>3- Add &lt;?php echo ml2subform(); ?&gt; in any template to show the form</P>
										<p>There are extra options you can add. Such as: [ml2subform nametxt="Name:" nameholder="Name..."] Below is an explanation of what each option does:</p>
				                            <ul>
				                              ...
				                              <li> <strong>&quot;phonetxt&quot;	</strong>-&gt;	Text that is displayed to the left of the phone input field.</li>
				                              <li> <strong>&quot;phoneholder&quot;	</strong>-&gt;	Text that is displayed inside the phone input box as a place holder.</li>
